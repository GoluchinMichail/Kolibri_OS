...
format PE DLL native 0.05
	entry START ; my_style 2014.11.30
...
proc AddDevice stdcall uses ebx esi edi, .config_pipe:DWORD, .config_descr:DWORD, .interface:DWORD
        ...
        mov     edx, eax
        DEBUGF 2,'K :ftdi: Detected device vendor: 0x%x\n', [eax+usb_descr.idVendor]
        ...
        jmp     .nothing
  @@:
        DEBUGF 2,'K :ftdi: Adding struct to list 0x%x\n', eax
        call    linkedlist_add
        ...
        DEBUGF 2, 'K :ftdi: Chip type 0x%x\n', ecx
        cmp     cx, 0x400
        jnz     @f
  ...
  .notftdi:
        DEBUGF 1,'K :ftdi: Skipping not FTDI device\n'
  .nothing:
        xor     eax, eax
        ret
endp
...
endl
        mov     edi, [ioctl]
        mov     eax, [edi+io_code]
        DEBUGF 1,'K :ftdi: FTDI got the request: %d\n', eax
        test    eax, eax            ;0
        jz      .version
        dec     eax                 ;1
        jz      .ftdi_get_list
  ...
  .version:
        jmp     .endswitch
  .error:
        DEBUGF 1, 'K :ftdi: FTDI error occured! %d\n', eax
        ;mov     esi, [edi+output]
        ;mov     [esi], dword 'ERR0'
        ;or      [esi], eax
        ret
  ...
  .own_index:
        mov     ebx, [edi+4]
        DEBUGF 2,'K :ftdi: ConfPacket 0x%x 0x%x\n', [ConfPacket], [ConfPacket+4]
        lea     esi, [ConfPacket]
        lea     edi, [EventData]
  ...
  .ftdi_setrtshigh:
        DEBUGF 2,'K :ftdi: FTDI Setting RTS pin HIGH PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
  ...
  .ftdi_setrtslow:
        DEBUGF 2,'K :ftdi: FTDI Setting RTS pin LOW PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
                                 + (SIO_SET_MODEM_CTRL_REQUEST shl 8) \
  ...
  .ftdi_setdtrhigh:
        DEBUGF 2,'K :ftdi: FTDI Setting DTR pin HIGH PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
                                 + (SIO_SET_MODEM_CTRL_REQUEST shl 8) \
  ...
  .ftdi_setdtrlow:
        DEBUGF 2,'K :ftdi: FTDI Setting DTR pin LOW PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
                                 + (SIO_SET_MODEM_CTRL_REQUEST shl 8) \
  ...
  .ftdi_usb_reset:
        DEBUGF 2,'K :ftdi: FTDI Reseting PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_purge_rx_buf:
        DEBUGF 2, 'K :ftdi: FTDI Purge TX buffer PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
                                 + (SIO_RESET_REQUEST shl 8) \
  ...
  .ftdi_purge_tx_buf:
        DEBUGF 2, 'K :ftdi: FTDI Purge RX buffer PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
                                 + (SIO_RESET_REQUEST shl 8) \
  ...
  .ftdi_set_bitmode:
        DEBUGF 2, 'K :ftdi: FTDI Set bitmode 0x%x, bitmask 0x%x %d PID: %d Dev handler 0x0x%x\n', \
  ...
  .ftdi_set_line_property:
        DEBUGF 2, 'K :ftdi: FTDI Set line property 0x%x PID: %d Dev handler 0x0x%x\n', \
                                                          [edi+8]:4,[edi],[edi+4]
        mov     word[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_set_latency_timer:
        DEBUGF 2, 'K :ftdi: FTDI Set latency %d PID: %d Dev handler 0x0x%x\n', \
                                                          [edi+8],[edi],[edi+4]
        mov     word[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_set_event_char:
        DEBUGF 2, 'K :ftdi: FTDI Set event char %c PID: %d Dev handler 0x0x%x\n', \
                                                          [edi+8],[edi],[edi+4]
        mov     word[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_set_error_char:
        DEBUGF 2, 'K :ftdi: FTDI Set error char %c PID: %d Dev handler 0x0x%x\n', \
                                                          [edi+8],[edi],[edi+4]
        mov     word[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_setflowctrl:
        DEBUGF 2, 'K :ftdi: FTDI Set flow control PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_OUT_REQTYPE) \
  ...
  .ftdi_read_pins:
        DEBUGF 2, 'K :ftdi: FTDI Read pins PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                     [edi+4]
        mov     ebx, [edi+4]
  ...
  .ftdi_set_wchunksize:
        DEBUGF 2, 'K :ftdi: FTDI Set write chunksize %d bytes PID: %d Dev handler 0x0x%x\n', \
                                                    [edi+8], [edi], [edi+4]
        mov     ebx, [edi+4]
        mov     ecx, [edi+8]
  ...
  .ftdi_get_wchunksize:
        DEBUGF 2, 'K :ftdi: FTDI Get write chunksize PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     esi, [edi+output]
        mov     edi, [edi+input]
        mov     ebx, [edi+4]
  ...
  .ftdi_set_rchunksize:
        DEBUGF 2, 'K :ftdi: FTDI Set read chunksize %d bytes PID: %d Dev handler 0x0x%x\n', \
                                                    [edi+8], [edi], [edi+4]
        mov     ebx, [edi+4]
        mov     ecx, [edi+8]
        cmp     [ebx + ftdi_context.maxPacketSize], ecx
  ...
  .ftdi_get_rchunksize:
        DEBUGF 2, 'K :ftdi: FTDI Get read chunksize PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     esi, [edi+output]
        mov     edi, [edi+input]
        mov     ebx, [edi+4]
  ...
  .ftdi_write_data:
        DEBUGF 2, 'K :ftdi: FTDI Write %d bytes PID: %d Dev handler 0x%x\n', [edi+8],\
                                                            [edi], [edi+4]
        mov     esi, edi
        add     esi, 12
        mov     ebx, [edi+4]
  ...
  .ftdi_read_data:
        DEBUGF 2, 'K :ftdi: FTDI Read %d bytes PID: %d Dev handler 0x%x\n', [edi+8],\
                                                            [edi], [edi+4]
        mov     edi, [ioctl]
        mov     esi, [edi+input]
        mov     edi, [edi+output]
  ...
  .ftdi_poll_modem_status:
        DEBUGF 2, 'K :ftdi: FTDI Poll modem status PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     ebx, [edi+4]
        mov     dword[ConfPacket], (FTDI_DEVICE_IN_REQTYPE) \
  ...
  .ftdi_get_latency_timer:
        DEBUGF 2, 'K :ftdi: FTDI Get latency timer PID: %d Dev handler 0x0x%x\n', [edi],\
                                                                        [edi+4]
        mov     ebx, [edi+4]
        mov     dword[ConfPacket], FTDI_DEVICE_IN_REQTYPE \
  ...
  .ftdi_get_list:
        DEBUGF 2, 'K :ftdi: FTDI devices" list request\n'
        mov     edi, [edi+output]
        xor     ecx, ecx
  ...
  .ftdi_lock:
        DEBUGF 2, 'K :ftdi: FTDI Lock PID: %d Dev handler 0x0x%x\n', [edi], [edi+4]
        mov     esi, [edi+input]
        mov     ebx, [esi+4]
  ...
  .ftdi_unlock:
        DEBUGF 2, 'K :ftdi: FTDI Unlock PID: %d Dev handler 0x0x%x\n', [edi], [edi+4]
        mov     esi, [edi+input]
        mov     edi, [edi+output]
        mov     ebx, [esi+4]
        mov     eax, [ebx + ftdi_context.lockPID]
...
proc DeviceDisconnected stdcall uses  ebx esi edi, .device_data:DWORD
        ...
        DEBUGF 1, 'K :ftdi: FTDI deleting device data 0x%x\n', [.device_data]
        mov     eax, [.device_data]
