...
int my_usb_control_msg (const char *proc, int line, struct usb_device *dev, unsigned int pipe,
    ...
    dev_info (&dev->dev, "MY %i = usb_control_msg (\"%s\".%i, pipe= %Xh, request= %Xh, reqtype= %Xh, value= %Xh, index= %Xh,   size= %u)\n"
        ,iResult
        ,proc,line
        /*dev*/, pipe
        ,request, requesttype, value, index
...
static struct usb_serial_driver ftdi_sio_device = {
	...
	.description =		"FmyTDI USB Serial Device",
...
static __u32 ftdi_232bm_baud_base_to_divisor(int baud, int base)
{
	...
	if (divisor == 1)
		...
		divisor = 1;
dev_info (my_dev, "ftdi_232bm_baud_base_to_divisor (%u, %u)= %Xh\n", baud, base, divisor);
	return divisor;
}
...
dev_info (dev, "get_ftdi_divisor 1\n");
	if (!baud)
		baud = 9600;
...
dev_info (dev, "get_ftdi_divisor 2\n");
	case FTX:     /* FT-X series */
		if (baud <= 3000000) {
			__u16 product_id = le16_to_cpu(
				port->serial->dev->descriptor.idProduct);
dev_info (dev, "get_ftdi_divisor 3\n");
...
dev_info (dev, "get_ftdi_divisor 4\n");
my_dev = dev;
